...
//PCBTEST::::: Errors FOUND
//          1) When you call free(), it does free up memory, but you can still print PCB
//          1) IN Create() if you do not put in string for name, code execution fails)
//          1) IN PCB Create() if you don't put in a int for PID, it returns some random int
